...
func decide_behavior(cell: Node2D, delta: float, time: float) -> void:
	...
	# ✅ EXISTING DNA KEY
	var speed_mod: float = cell.genetics.dna["speed"]
	...
	if aggression > 0.6:
		...
		if nearest_cell and nearest_dist < 600.0:
			...
			cell.global_position += (
				cell.global_position.direction_to(cell.target_pos)
				* speed * delta
			)
			return
	...
	if cell.shared_target != Vector2.ZERO \
	and cell.global_position.distance_to(cell.shared_target) > 50.0:
		cell.global_position += (
			cell.global_position.direction_to(cell.shared_target)
			* speed * delta
		)
		return
